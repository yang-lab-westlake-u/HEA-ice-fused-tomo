...
def save_data(folder_name, chem_original, chem_fused, haadf_original, haadf_fused, eList, nx, ny, costHAADF, costChem,
              costTV, lambdaHAADF, lambdaChem, lambdaTV, gamma):
    import h5py
    import os
    from skimage.io import imsave
    ...
    file = h5py.File(os.path.join(folder_name, 'Fused_Reconstruction.h5'), 'w')
    file.create_dataset('raw_haadf', data=haadf_original.reshape(nx, ny))  # 
    file.create_dataset('fused_haadf', data=haadf_fused.reshape(nx, ny))  # 
    imsave(os.path.join(folder_name, 'haadf_recon.tif'), haadf_fused.reshape(nx, ny))  # 

    for ii in range(len(eList)):
        chem_original_image = chem_original[ii * (nx * ny):(ii + 1) * (nx * ny)].reshape(nx, ny)
        chem_fused_image = chem_fused[ii * (nx * ny):(ii + 1) * (nx * ny)].reshape(nx, ny)

        # 
        chem_fused_image_adjusted = exposure.rescale_intensity(chem_fused_image)

        file.create_dataset('raw_' + eList[ii], data=chem_original_image)
        file.create_dataset('fused_' + eList[ii], data=chem_fused_image_adjusted)

        # 
        imsave(os.path.join(folder_name, f'{eList[ii]}_signal.tif'), img_as_uint(chem_fused_image_adjusted))

    file.create_dataset('costHAADF', data=costHAADF)
    file['lambdaHAADF'] = lambdaHAADF
    file.create_dataset('costChem', data=costChem)
    file['lambdaChem'] = lambdaChem
    file.create_dataset('costTV', data=costTV)
    file['lambdaTV'] = lambdaTV
    file['gamma'] = gamma
    file.close()
...
#
# 		# 
# 		imsave(folder_name + '/{}_signal.tif'.format(eList[ii]), img_as_uint(chem_fused_image_adjusted))
#
# 	file.create_dataset('costHAADF', data=costHAADF)
# 	file['lambdaHAADF'] = lambdaHAADF
# 	file.create_dataset('costChem', data=costChem)
...
def plot_elemental_images(data,haadf,eList,nx,ny,nrows,ncols):
	...
	ax[0].axis('off')
	for ii in range(len(eList)):
		ax[ii+1].imshow(data[ii*(nx*ny):(ii+1)*(nx*ny)].reshape(nx,ny),cmap='gray')
		ax[ii+1].set_title(eList[ii])
		ax[ii+1].axis('off')
	plt.show()
